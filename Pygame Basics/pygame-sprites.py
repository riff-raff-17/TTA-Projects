...
PLAYER_IMG  = pygame.image.load(os.path.join(GAME_FOLDER, 'player.png')).convert_alpha()
BLOCK_IMG   = pygame.image.load(os.path.join(GAME_FOLDER, 'block.png' )).convert_alpha()

# --- Sprite Classes ---
class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        # use the image you loaded
...
class Block(pygame.sprite.Sprite):
    def __init__(self, speed):
        super().__init__()
        # pick a random size, then scale your block image to that
        size = random.randint(20, 50)
        self.image = pygame.transform.scale(BLOCK_IMG, (size, size))
